...
public class QueryParser {
  ...
  public static Query parse(Layer layer, String sql) {
    ...
    String select = matcher.group(1).strip();
    String from = matcher.group(2).strip();
    Optional<String> where = Optional.ofNullable(matcher.group(3)).map(s -> s.strip());
    ...
    String tags = columns[1].strip();
    String geom = columns[2].strip();
